...
export default {
  ...
  methods: {
    getPages () {
      this.$axios.get(`http://localhost:3000/api/manage/page`)
        .then((r) => {
    ...
    putPage () {
      this.dialog = false
      this.$axios.put(`http://localhost:3000/api/manage/page/${this.putId}`, {
    ...
    delPage (id) {
      this.$axios.delete(`http://localhost:3000/api/manage/page/${id}`)
        .then((r) => {
          this.pop('페이지 삭제 완료')
          this.getPages()
        })
